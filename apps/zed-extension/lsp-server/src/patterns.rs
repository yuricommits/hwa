...
pub static PATTERNS: LazyLock<Vec<Pattern>> = LazyLock::new(|| {
    vec![
        // ── Cryptography ──────────────────────────────────────────────
        Pattern {
            regex: Regex::new(r#"createHash\s*\(\s*['"]md5['"]\s*\)"#).unwrap(),
        ...
        Pattern {
            regex: Regex::new(r"hashlib\.md5\s*\(").unwrap(),
            description: "MD5 is cryptographically broken",
        ...
        Pattern {
            regex: Regex::new(r#"createHash\s*\(\s*['"]sha1['"]\s*\)"#).unwrap(),
        ...
        Pattern {
            regex: Regex::new(r"hashlib\.sha1\s*\(").unwrap(),
            description: "SHA1 is deprecated for security use by NIST",
        ...
        // ── SQL Injection ─────────────────────────────────────────────
        Pattern {
            regex: Regex::new(r"(?i)SELECT\s+.*\s+FROM\s+\w+\s+WHERE\s+.*\$\{").unwrap(),
        ...
        Pattern {
            regex: Regex::new(r#"(?i)(?:execute|cursor\.execute)\s*\(\s*f["'].*(?:username|password|email|id|input)"#).unwrap(),            description: "Potential SQL injection — f-string in SQL query",
        ...
        // ── Command Injection ─────────────────────────────────────────
        Pattern {
            regex: Regex::new(r"subprocess\.(?:run|call|Popen)\s*\(.*shell\s*=\s*True").unwrap(),
        ...
        // ── SSL/TLS ───────────────────────────────────────────────────
        Pattern {
            regex: Regex::new(r"verify\s*=\s*False").unwrap(),
        ...
        Pattern {
            regex: Regex::new(r#"(?i)(?:api_key|apikey|api-key)\s*(?:=|:)\s*['"`][^'"`]{8,}['"`]"#).unwrap(),
        ...
        Pattern {
            regex: Regex::new(r"eyJ[a-zA-Z0-9_-]{10,}\.[a-zA-Z0-9_-]{4,}(?:\.[a-zA-Z0-9_-]*)?").unwrap(),
        ...
        Pattern {
            regex: Regex::new(r"(?:AKIA|AIPA|ASIA|AROA|AIDA)[A-Z0-9]{16}").unwrap(),
        ...
        // ── Weak Randomness ───────────────────────────────────────────
        Pattern {
            regex: Regex::new(r"Math\.random\s*\(\s*\)").unwrap(),
        ...
        Pattern {
            regex: Regex::new(r"random\.random\s*\(\s*\)").unwrap(),
        ...
        // ── Debug Mode ────────────────────────────────────────────────
        Pattern {
            regex: Regex::new(r"app\.run\s*\(.*debug\s*=\s*True").unwrap(),
        ...
        // ── Rust ──────────────────────────────────────────────────────
        Pattern {
            regex: Regex::new(r#"(?i)(?:api_key|apikey|api-key|secret|password|token)\s*=\s*"[^"]{4,}""#).unwrap(),
        ...
        Pattern {
            regex: Regex::new(r#"\.expect\s*\(\s*"[^"]*"\s*\)"#).unwrap(),
        ...
        Pattern {
            regex: Regex::new(r"rand::random\s*::<").unwrap(),
